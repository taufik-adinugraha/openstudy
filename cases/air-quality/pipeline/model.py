...
DROP = {"ts_utc", "persistence", "wind_from_sector", "date", "n_sub"}
...
def fit_horizon(train: pd.DataFrame, test: pd.DataFrame, feats: list[str], h: int,
                clim: pd.Series) -> tuple[dict, pd.DataFrame, object]:
    ycol = f"y_h{h}"
...
def main() -> None:
    ...
    ts = df["ts_utc"].drop_duplicates().sort_values()
    cut = ts.iloc[int(len(ts) * (1 - config.TEST_FRACTION))]
    train, test = df[df["ts_utc"] < cut], df[df["ts_utc"] >= cut]
    log(f"time split at {cut} — train {len(train):,} rows to {train['ts_utc'].max()}, "
        f"test {len(test):,} rows from {test['ts_utc'].min()}")
    ...
    for h in config.HORIZONS:
        row, p, m = fit_horizon(train, test, feats, h, clim)
        if not row:
            continue
        rows.append(row)
        preds.append(p)
    ...
    r = permutation_importance(models[h_imp], sample[feats], np.log1p(sample[f"y_h{h_imp}"].clip(lower=0)),
                               n_repeats=5, random_state=0, scoring="neg_mean_absolute_error")
    imp = (pd.DataFrame({"feature": feats, "importance": r.importances_mean, "sd": r.importances_std})
    ...
    for loc, g in df.groupby("location_id"):
        ...
        for h, m in models.items():
            if m is None:
                continue
            v = float(np.expm1(m.predict(last[feats])[0]))
